...
with st.sidebar:
    ...
    try:
        ...
        st.warning("Could not connect to Pinecone.")
...
# -----------------------------
st.markdown(
    '<div class="main-header">🔍 AI Resume Screener</div>',
    unsafe_allow_html=True
)
...
if search_clicked:
    ...
    if not job_description.strip():
        ...
        if results_df.empty:
            ...
            st.markdown(
                f"### 🏆 Top {len(results_df)} Candidates"
            )
            ...
            for _, row in results_df.iterrows():
                ...
                with st.expander(
                    f"#{row['rank']} — {row['category']} | {score_pct} match"
                ):
                    ...
                    with col2:
                        ...
                        st.caption(f"ID: {row['candidate_id'][:20]}")
